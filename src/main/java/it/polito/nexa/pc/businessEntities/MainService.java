...
public class MainService {
	...
	public static void main(String[] args) {
	    ...
	    // Read all json files 
	        ...
	        File dir = new File("C:/pythonexercises/download/");
	        ...
	        while (itr.hasNext()) {
	            ...
	            if(!fileName.equals("businessEntities.json") &&!fileName.equals("stats.json") && !fileName.equals("proposingStructure.json") && !fileName.equals("downloadInfo.json")&& !fileName.equals("downloadStats.json") && !fileName.contains("_index")){
	        ...
	        //remove from the list, all the public administrations
	        removePA("download/proposingStructures.json");
	        
	        LinkedList<HashMap<String, Object>> result =  companies.values().stream()
	        .collect(Collector.of(
	                () -> new LinkedList<HashMap<String, Object>>(),          // supplier
	                (list, c) ->     {
	                	Company company = (Company) c; 
	                	HashMap <String, Object> res = new HashMap<>(); 
	                	res.put("id", company.getId()); 
	                	if (company.hasOriginalId()) res.put("originalId", company.getOriginalIds()); 
	                	res.put("name", company.getBestLabel()); 
	                	res.put("isItalian", company.isItalian());
	                	//uncomment this to write all labels (debug)
	                	//res.put("name", company.getAllLabels()); 
	                	list.add(res);
	                /*System.out.println(p); */ } ,  // accumulator
	                (j1, j2) ->{ j1.addAll(j2); 
	                return j1; })); 
	...
	private static void   companiesFromJson(String inputJSON){
        ...
        try {
            ...
            //companies.values().forEach(company-> System.out.println(company.getBestLabel()));
        }catch (Exception e) {
	            e.printStackTrace();
	...
	private static void addCompany (JsonNode record){
		...
		if(getValue("companyHash", record) != "") {
        	...
        	//if (id.length()<3) System.out.println(record);
        }
		
		//add company to the list 
		if (originalId==""){
			if (companies.containsKey(id)) companies.get(id).addName(name);
	...
	private static void addGroup(JsonNode record){
		JsonNode members ; 
		if (record.has("aggiudicatarioRaggruppamento")) members =record.get("aggiudicatarioRaggruppamento"); 
